...
import { ALL_POSITIVE_TRANSACTIONS } from "../../../graphql/transactionQueries";
...
const AllPositiveTransactionsPage: React.FC = () => {
  const router = useRouter();
  const { loading, error, data } = useQuery(ALL_POSITIVE_TRANSACTIONS);
  ...
  const { allPositiveTransactions }: { allPositiveTransactions: Transaction[] } = data;

  return (
    <div>
      <AllTransactionsNav />
      <div>
        {allPositiveTransactions.map((transaction) => {
...
export default withApollo({ ssr: true })(AllPositiveTransactionsPage);
